...
**`이정원`**
- 영화 상세 페이지
  - 영화 정보(평균 평점, 영화 커버, 상세 정보, 소개 영상, OTT제공 정보, 비슷한 영화 추천), 리뷰 목록 표시
  - 평점 및 리뷰 CRUD, 리뷰 좋아요 등록 및 해제, 와치리스트(찜) 등록 및 해제
  - 로그인 한 사용자와 로그인 안한 사용자 api요청과 버튼 조작 로직 분리

- 마이 페이지
  - 프로필 이미지(5MB이하) 업로드 및 미리보기
  - 회원정보 및 비밀번호 수정 Form 분리하여 렌더링 최적화
  - 내가 남긴 리뷰 모아보기

- 통계 페이지
  - 사용자가 선택한 연도의 연간 개봉 장르 통계 데이터 api 요청 및 차트 생성
